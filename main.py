...
if sys.argv[1] == 'DreamTime':
    ...
    # we now need to ensemble each iteration of inception (aka DreamTime)
    archive_name = ARCHIVE_NAMES[0]
    classifier_name = 'nne'
    ...
    clfs = []
    itr = '-0-1-2-3-4-'
    dreamTime = 'nne/inception'
    # add DreamTime: an ensemble of 5 Inception networks
    clfs.append(dreamTime + itr)
    # add DreamTime for each hyperparameter study
    for xp in xps:
        xp_arr = get_xp_val(xp)
        for xp_val in xp_arr:
            clfs.append(dreamTime + '/' + xp + '/' + str(xp_val) + itr)
